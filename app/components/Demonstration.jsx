...
export default function Demonstration() {
  ...
  const triggerChatSubmit = (userText) => {
    ...
    setTimeout(() => {
      ...
      if (activeTab === "leads") {
        aiResponse =
          "Your results have been collected, you can view them in the view leads page.";
      } else if (activeTab === "report") {
  ...
  const generateCampaignEmail = () => {
    ...
    let hook =
      campaignSubject === "discount"
        ? "We are currently offering a 20% discount on all audits this month."
        : campaignSubject === "offer"
          ? "I wanted to share an exclusive offer we have tailored for your business."
          : "We have a crucial update regarding your account processes that I wanted to share.";

    let push =
      campaignGoal === "phone call"
        ? "Are you available for a brief phone call next Tuesday to discuss this?"
        : campaignGoal === "visit link"
          ? "Please visit the link below to claim this opportunity before it expires."
      ...
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Experience the power of our conversational and targeted AI workflows
            effortlessly. Watch the automation or interact with our template
            builders below.
          </p>
        </div>
              ...
              <div className="w-full max-w-3xl bg-white h-[500px] sm:rounded-2xl shadow-sm border border-slate-200 flex flex-col pointer-events-none">
                  ...
                  <AnimatePresence>
                    {messages.map((msg) => (
                      ...
                      >
                        ...
                        <div
                          className={`p-4 rounded-xl max-w-[80%] ${msg.sender === "user" ? "bg-indigo-600 text-white rounded-tr-none" : "bg-slate-100 text-slate-800 rounded-tl-none"}`}
                        >
                ...
                <div className="p-4 bg-white border-t border-slate-100 rounded-b-2xl">
                  <form
                    ...
                    className="flex gap-2 relative shadow-sm rounded-full overflow-hidden border border-slate-300 bg-slate-50 p-1"
                  >
                    <input
                      type="text"
                      readOnly
                      ...
                      placeholder={
                        activeTab === "leads"
                          ? "e.g., Target 100 construction companies in Nairobi"
                          : "e.g., Write a report on https://example.com"
                      }
                    ...
                    <button
                      type="button"
                      disabled
                      className="w-10 h-10 bg-indigo-400 text-white flex justify-center items-center rounded-full transition disabled:opacity-50"
                    >
              ...
              <div className="w-full max-w-xl mx-auto pointer-events-none">
                ...
                >
                  ...
                  <button
                    type="button"
                    disabled
                    className="w-full py-3 bg-indigo-400 text-white font-semibold rounded-xl transition flex justify-center items-center mt-4 h-12"
                  >
                    {isSearchingEmail ? (
                      <Loader size={20} className="text-white border-white" />
                    ) : (
                      "Find Contact"
                    )}
                  </button>
                ...
                {emailResult && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mt-6 bg-white p-6 rounded-2xl border border-green-200 shadow-md"
                  >
